...
// Importiamo le nostre costanti e utility locali
import { OCTAVE_BANDS } from '../constants/frequencies.js';
...
export const ThreeDUtils = {
    ...
    analyzeGeometry: (model) => {
        ...
        const areaScaleFactor = model.scale.x * model.scale.y;
        ...
        model.traverse((child) => {
            if (child.isMesh && child.geometry) {
                ...
                const surfaceInfo = ThreeDUtils.categorizeSurfaces(geometry);
    ...
    calculateRoomAcoustics: (roomModel, materialAssignments) => {
        ...
        const frequencyBands = OCTAVE_BANDS;
        ...
        frequencyBands.forEach(freq => {
            ...
            for (const type in surfacesByType) {
                ...
                if (materialKey) {
                    const absorptionCoeff = MaterialUtils.getAbsorptionAtFrequency(materialKey, freq);
        ...
        const rt60PerBand = {};
        frequencyBands.forEach(freq => {
            rt60PerBand[freq] = Calculations.sabineRT60(volume, totalAbsorptionPerBand[freq]);
        });
